...
除非连接主动要求，SQLite 默认不启用外键，因此 `beforeOpen` 会打开约束检查，属于某首曲目或某个播放列表的行（收藏、歌词缓存、播放事件、播放列表条目）会随之级联删除。在此之前，删除一首曲目会把这些行留下：凡是需要解析曲目 id 的地方它们都会消失，但那些独立聚合播放事件的统计仍在计入它们，于是总计数字与其下方列出的曲目对不上。
