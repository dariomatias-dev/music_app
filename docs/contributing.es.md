...
Esa última línea apunta git a [`.githooks/`](../.githooks), donde un hook `commit-msg` rechaza un asunto que no sigue la convención descrita más abajo, y un hook `pre-push` ejecuta el control de calidad antes de que un push salga de la máquina. Git no comparte los hooks al clonar, así que es un comando por cada copia local.
...
La app abre con una biblioteca vacía hasta que un escaneo encuentra archivos de audio en el dispositivo. Para trabajar sobre una app poblada sin depender de eso, ejecútala con los seeds de desarrollo:

```sh
fvm flutter run --dart-define=SEED_ENABLED=true
```

Los seeds escriben artistas, álbumes, pistas, playlists, favoritos, letras en caché, historial de escucha, búsquedas recientes y carpetas excluidas, fechados con respecto al momento en que se ejecutan. Una nueva ejecución reemplaza esas filas en lugar de añadir una segunda copia, y deja intacto el resto de la base de datos.

El flag es de tiempo de compilación y los seeds además se niegan a ejecutarse fuera del modo debug, de modo que ni los datos de ejemplo ni el código que los escribe llegan a una build de release.

`dart run scripts/seed.dart` escribe un archivo de base de datos poblado en `build/seed/`, sin dispositivo y sin build.
...
`main` está protegida: no acepta push directo, ni push forzado, ni borrado, y un pull request se fusiona solo cuando el CI está en verde. Trabaja en una rama, abre el pull request y deja que se fusione solo:
...
`--auto` deja la fusión en cola para el momento en que pasen las comprobaciones requeridas, así que no hace falta vigilar el pull request. La rama también tiene que estar al día con `main` antes de fusionarse.

Tres comprobaciones bloquean la fusión: `Vulnerabilities`, `music_app` y `packages/app_ui`. `Build APK` e `Integration tests` se ejecutan en cada pull request e informan, pero no lo bloquean: ambos esperan a `music_app` para empezar, y exigirlos pondría un arranque de emulador delante de cada fusión. Lo que cubren no se pierde, porque [`release.yml`](../.github/workflows/release.yml) ejecuta el control completo y la compilación de release antes de publicar nada.

El hook `pre-push` instalado durante la configuración ejecuta antes `./scripts/verify.sh --all` en local, lo que convierte una comprobación fallida en una respuesta inmediata en vez de en un pull request en rojo. Ejecuta el control sin acotar a propósito: `verify.sh` deriva su alcance de los cambios pendientes en el árbol de trabajo, y al hacer push esos cambios ya son commits, así que una ejecución acotada no encontraría nada que comprobar. `git push --no-verify` lo omite; lo que realmente sostiene es la protección de rama.
...
[`.github/workflows/release.yml`](../.github/workflows/release.yml) ejecuta entonces las mismas verificaciones que CI y adjunta la APK de release. Lo invoca directamente [`release_please.yml`](../.github/workflows/release_please.yml), ya que GitHub no arranca un workflow a partir de una etiqueta subida con el token por defecto, y sigue respondiendo a una etiqueta `v*.*.*` subida a mano, creando el release por su cuenta en ese caso. Ten en cuenta que la compilación de release se firma con la **keystore de debug** a propósito, pues esta app no tiene configuración de firma de producción.
...
`-j` recibe el **id** del job (`vulnerabilities`, `app`, `build_apk`, `integration`, `app_ui`, `release`), no el nombre visible de la tabla de arriba; `act -l` muestra ambos. La primera ejecución real descarga una imagen de runner de varios gigabytes, y `act` aproxima los runners de GitHub en lugar de reproducirlos exactamente, así que un `act` en verde es una buena señal, no una garantía.
...
[Renovate](https://docs.renovatebot.com/) está configurado en [`renovate.json`](../renovate.json) y abre pull requests semanales para pub, Gradle y GitHub Actions, siguiendo ambos archivos `pubspec.yaml`. También mantiene una issue de Dependency Dashboard que lista todo lo que está reteniendo y por qué.

Los pines de [`dependencies.md`](dependencies.es.md) están codificados ahí en lugar de dejarse para detectar en revisión: `drift`, `sqlite3`, `intl` y `flutter_rust_bridge` están deshabilitados por completo, Gradle y el Android Gradle Plugin quedan topados por debajo de su línea 9.x, y los paquetes de Riverpod se agrupan para que la cadena se mueva junta. Eso reemplaza el ritual semanal de cerrar pull requests que los pines prohíben.

Esos pull requests pasan por el mismo CI que cualquier otro. Cuando se levante un pin, elimina su regla de `renovate.json` en el mismo cambio que actualice [`dependencies.md`](dependencies.es.md).
